...
// chat thread (see api/relay-call-status.js) - unrelated to placing or
// running the call itself.
// UNVERIFIED: never exercised against a live call.
export async function wacallsAttachAI(userId, sessionId, callId, { appSessionId, contactName, peerNumber } = {}) {
  return wacallsRequest(`/api/sessions/${sessionId}/calls/${callId}/ai`, {
    userId,
    method: 'POST',
    body: { userId, sessionId: appSessionId, contactName, peerNumber },
  });
}
